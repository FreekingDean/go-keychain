...
// GetACLList is new
func GetACLList(item Item) ([]ACL, error) {
	itemRef, err := QueryItemRef(item)
	if err != nil {
		return []ACL{}, err
	}
	defer Release(itemRef)
	var accessRef C.SecAccessRef
	errCode := C.SecKeychainItemCopyAccess(C.SecKeychainItemRef(itemRef), &accessRef)
	err = checkError(errCode)
	if err != nil {
		return []ACL{}, err
	}
	var acls C.CFArrayRef
	errCode = C.SecAccessCopyACLList(accessRef, &acls)
	defer Release(C.CFTypeRef(accessRef))
	err = checkError(errCode)
	if err != nil {
		return []ACL{}, err
	}
	aclRefs := CFArrayToArray(acls)

	typedACLs := make([]ACL, 0)
	for _, acl := range aclRefs {
		var applications C.CFArrayRef
		var description C.CFStringRef
		var promptSelector C.ushort
		aclRef := C.SecACLRef(acl)
		errCode = C.SecACLCopyContents(aclRef, &applications, &description, &promptSelector)
		if err = checkError(errCode); err != nil {
			return []ACL{}, err
		}
		application := ""
		...
		if applications != 0 {
			for _, applicationRef := range CFArrayToArray(applications) {
				var data C.CFDataRef
				errCode = C.SecTrustedApplicationCopyData(C.SecTrustedApplicationRef(applicationRef), &data)
				if err = checkError(errCode); err != nil {
					return []ACL{}, err
				}
				typedData, err := CFDataToBytes(data)
				if err != nil {
					return []ACL{}, err
				}
				application = string(typedData)
			}
		}
		typedACLs = append(typedACLs,
			ACL{
				Description: CFStringToString(description),
				Application: application,
			},
		)
		//Release(C.CFTypeRef(description))
		//Release(C.CFTypeRef(applications))
	}
	return typedACLs, nil
}

type Owner struct{}

// GetACLOwner is new
func GetACLOwner(item Item) Owner {
	return Owner{}
}
